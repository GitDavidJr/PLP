% Fatos: Livros e suas caracteristicas
...
% Regra para calcular a correspondencia com base nas preferencias fornecidas
calcular_probabilidade(Preferencias, CaracteristicasLivro, Probabilidade) :-
    % Extrair todos os generos do livro
    findall(Genero, member(genero(Genero), CaracteristicasLivro), GenerosLivro),
    % Contar quantas preferencias sao atendidas pelos generos do livro
    findall(1, (member(Pref, Preferencias), member(Pref, GenerosLivro)), ListaMatches),
    length(ListaMatches, NumMatches),
    length(Preferencias, TotalPreferencias),
    % Calcular a probabilidade como a razao entre matches e o total de preferencias
    Probabilidade is (NumMatches / TotalPreferencias) * 100. % Multiplicar por 100 para obter porcentagem

% Regra principal: encontrar recomendacoes com base nas preferencias
recomendacao(Preferencias, Livro, Probabilidade) :-
    livro(Livro, CaracteristicasLivro),
    calcular_probabilidade(Preferencias, CaracteristicasLivro, Probabilidade),
    Probabilidade > 0.

% Consultas auxiliares para verificar as caracteristicas de um livro
genero(Livro, Genero) :- livro(Livro, Caracteristicas), member(genero(Genero), Caracteristicas).
autor(Livro, Autor) :- livro(Livro, Caracteristicas), member(autor(Autor), Caracteristicas).
ano(Livro, Ano) :- livro(Livro, Caracteristicas), member(ano(Ano), Caracteristicas).
